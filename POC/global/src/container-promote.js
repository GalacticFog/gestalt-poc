// This promotion function does the following:
//  - Updates the existing container in the target enviornment if it exists, or
//  - Creates a new container in the target environment
//    - Set num_instances = 1 (or 0 if original was scaled down) even if original instances was > 1
//    - Sets the container description to indicate the promotion
//
...
function promote(args, ctx) {
    ...
    if ( ! tgt_app ) {
        ...
        try {
            ...
            new_app = create_container(parent_org, tgt_env, {
                name:        cur_app.name,
                // description: cur_app.description,
                description: "Promoted at " + String(Date()),
                properties: {
                    provider: {
                        id: op.provider.id
                    },
    ...
    // Otherwise, modify the existing container
